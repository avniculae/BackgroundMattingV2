...
class Evaluator:
    ...
    def evaluate_worker(self, dataset, clip, position):
        ...
        for i, framename in enumerate(tqdm(framenames, desc=f'{dataset} {clip}', position=position, dynamic_ncols=True)):
            print(os.path.join(self.args.true_dir, dataset, clip, 'pha', framename))
...
def src_compose(fgr_dir, bgr_path, pha_dir):

    fgrs = sorted(os.listdir(fgr_dir))
    phas = sorted(os.listdir(pha_dir))
    assert len(fgrs) == len(phas)
    
    for i in range(len(fgrs)):
        fgr_path = os.path.join(fgr_dir, fgrs[i])
        pha_path = os.path.join(pha_dir, phas[i])
        true_fgr = Image.open(fgr_path).resize((600,400))
        true_pha = Image.open(pha_path).resize((600,400))
        true_bgr = Image.open(bgr_path).resize((600,400))
        # true_pha.convert("RGBA").show()
        # true_bgr.show()
        true_src = Image.composite(true_fgr, true_bgr, true_pha.convert("L"))
        true_src.save("inference_data/src_%s.jpg" % str(i).zfill(5), "JPEG")
        # true_bgr.save("inference_bgr/bgr_%s.jpg" % i, "JPEG")
def rename(dir):
    for file in os.listdir(dir):
        os.rename(os.path.join(dir, file), os.path.join(dir, 'src_' + file))

def crop_images(dir, size):
    for img in os.listdir(dir):
        image = Image.open(os.path.join(dir, img)).resize((size, size))
        image.save(os.path.join(dir, img))

if __name__ == '__main__':
    fgr_dir = "D:\\CS\\Part III\\mvp\\VideoMatte240K_JPEG_SD\\test\\fgr\\0000"
    pha_dir = "D:\\CS\\Part III\\mvp\\VideoMatte240K_JPEG_SD\\test\\pha\\0000"
    bgr_path = "D:\\CS\\Part III\\mvp\\Backgrounds\\church_interior_226.res-church.jpg"

    # src_compose(fgr_dir, bgr_path, pha_dir)
    Evaluator()
    # crop_images("D:\\CS\\Part III\\mvp\\BackgroundMattingV2\\Inference\\true_dir\\videomatte\\0000\\pha",256)
    # crop_images("D:\\CS\\Part III\\mvp\\BackgroundMattingV2\\Inference\\true_dir\\videomatte\\0000\\fgr",256)